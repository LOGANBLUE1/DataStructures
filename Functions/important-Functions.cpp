...
int printLIS(vector<int>& a) {
    int n = a.size(), maxi = 1, lastIndex = 0;
    vector<int>dp(n, 1), hash(n);
    iota(hash.begin(), hash.end(), 0);
    for(int i=0;i<n;i++)    for(int j=i+1;j<n;j++){
            if(a[j] > a[i] && dp[j] < dp[i]+1)  dp[j] = dp[i]+1, hash[j]=i;
            if(dp[j] > maxi)    maxi = dp[j], lastIndex = j;
        }
    vector<int>temp;
    temp.push_back(a[lastIndex]);
    while(hash[lastIndex]!=lastIndex)   lastIndex = hash[lastIndex], temp.push_back(a[lastIndex]);
...
int sumOfDigits(int num){
    int sum = 0;  string s = to_string(num);
    for (char digit : s)    sum += digit - '0';
    return sum;
}
...
double power(double x, int n) {
    ...
    long long nn = n;
    if(nn < 0)  nn = -nn;
    while (nn)  if (nn % 2)  ans *= x, nn--;    else    x *= x, nn >>= 1;
...
bool checkPalindrome(string a) {
    ...
    while(s<=e)     if(a[s] != a[e])    return 0;       else    s++, e--;
    return 1;
}
