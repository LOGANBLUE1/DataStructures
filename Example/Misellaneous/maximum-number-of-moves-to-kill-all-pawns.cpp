    ...
    while(!q.empty()) {
        ...
        for(int i = 0; i < 8; i++) {
            ...
            if(newx >= 0 && newx < 50 && newy >= 0 && newy < 50 && dist[startx][starty][newx][newy] == -1) {
                dist[startx][starty][newx][newy] = d + 1; // Update distance
...
class Solution {
public:
    vector<vector<vector<int>>> dp; // Memoization table for storing intermediate results
    ...
    int f(int curr, int mask, int turn, vector<vector<int>>& positions) {
        ...
        if(dp[curr][mask][turn] != -1) return dp[curr][mask][turn];
    ...
    int maxMoves(int kx, int ky, vector<vector<int>>& positions) {
        ...
        dp.resize(n, vector<vector<int>>(1 << n, vector<int>(2,-1))); // Initialize memoization table
